...
class ExpandedISO extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      contactMessage: notLoggedIn
        ? "You must login to contact this buyer"
        : "Contact",
      resolvedIndicator:
        props.props.iso.status === "resolved" ? (
      ...
      statusResolveMessage:
        props.props.iso.status === "resolved" ? "Re-Post" : "Unpost",
      name: "",
      editMode: false,
      coordinates: null,
    };
    ...
    this.ownerActions =
      this.props.props.iso._id === -1
        ? []
        : [
            {
              name: this.state.statusResolveMessage,
              icon: (
                <Icon name="check-circle" type="feather" color={ROOMER_GRAY} />
              ),
              onClick: this.handleOnChangeStatus,
            },
            {
              name: "Edit",
              icon: <Icon name="edit" type="feather" color={ROOMER_GRAY} />,
  ...
  ActionItem(props) {
    return (
      <TouchableOpacity style={styles.actionItem} onPress={props.onClick}>
        {props.icon}
        <>{props.name}</>
      </TouchableOpacity>
    );
  ...
  async handleOnSubmit(event) {
    ...
    if (
      ...
      //send email
      let toEmailValue = await getUserEmail(
        this.props.props.iso.userID,
        this.state.token
      );
      if (toEmailValue == -1) {
        //TODO: error handling
        ...
        let addMessageError = await addMessage(
          this.state.viewerId,
          this.props.props.iso._id,
          this.state.subjectValue,
          this.state.messageValue,
          this.props.props.iso.userID,
          this.state.token
  ...
  async handleOnChangeStatus(event) {
    ...
    let error =
      this.props.props.iso.status === "resolved"
        ? await unresolvePostStatus(
            this.props.props.iso.userID,
            this.props.props.iso._id,
            this.state.token
          )
        : await resolvePostStatus(
            this.props.props.iso.userID,
            this.props.props.iso._id,
            this.state.token
          );
    if (error == -1) {
      ...
      newIsoProps.iso.status =
        newIsoProps.iso.status === "resolved" ? "unresolved" : "resolved";
      this.setState({
        statusResolveMessage:
          this.state.statusResolveMessage === "Re-Post" ? "Unpost" : "Re-Post",
        props: newIsoProps,
      });
    }
  ...
  render() {
    return this.state.editMode ? (
      <AddPost
        props={{
          onPress: this.props.props.onPress,
          userID: this.state.viewerId,
          iso: this.props.props.iso,
      ...
      <View style={styles.expandedIsoContainer}>
        <View style={styles.expandedIsoInfoContainer}>
            ...
            <Image
              source={{
                uri:
                  `https://` +
                  getAWS_BUCKET_NAME() +
                  `.s3.us-east-2.amazonaws.com/${this.props.props.iso.userID}`,
              }}
              style={[styles.profileImageExpanded]}
            />
            <UserPageLink
              id={this.props.props.iso.userID}
              name={this.state.name}
              isOwner={this.state.viewerIsAuthor}
              exitCallback={this.props.props.onPress}
            />
          </View>
          <View style={[styles.isoContentContainerExpanded]}>
            ...
            <Text style={[styles.datePostedRow]}>
              Move-in:{" "}
              {epochToDateString(this.props.props.iso.startDate) +
                " - " +
                epochToDateString(this.props.props.iso.endDate)}
            </Text>
            {this.state.resolvedIndicator}
            <Text style={[styles.isoContentText]}>
        ...
        <div
          className={
            this.state.viewerIsAuthor ? "form-box-invisible" : "form-box"
          }
        >
...
const styles = StyleSheet.create({
  ...
  isoBorderExpanded: {
    width: !isMobile ? 360 : win.width * 0.9,
    borderColor: ROOMER_GRAY,
    borderBottomWidth: 1,
    height: 25,
  ...
  isoContentContainerExpanded: {
    ...
    width: !isMobile ? 650 : win.width * 0.9,
  },
  ...
  tagRowExpanded: {
    flexDirection: "row",
    width: !isMobile ? 500 : win.width * 0.9,
    flexWrap: "wrap",
    paddingBottom: !isMobile ? 0 : 20,
    marginHorizontal: !isMobile ? 0 : win.width * 0.05,
  },
